...
dfhark = pd.read_csv(f'osa_result/HARK_GDAXI_FCST.csv').iloc[-500:]
...
print(f"rmse: {round(rmsehark * 1000, 3)}")
# QLIKE
qlikehark = qlike(rvactualhark, rvpredictedhark)
print(f"qlike: {round(qlikehark * 100, 3)}")
# MAE
maehark = mae(rvactualhark, rvpredictedhark)
print(f"mae: {round(maehark * 1000, 3)}")  
