...
from rhark import log_likelihood_rhark, log_likelihood_rharkq
...
def mle_rharkq(log_rv, onv, filename):
    init_params = [0.001, 0.5, 0.5, 0.5, 0.1, 0.1]
    # est = minimize(
    ...
    ll = - result.fun
    aic = (2 * len(init_params)) - (2 * ll)
    # se = nmse(log_likelihood_rharkq, filename, log_rv=log_rv, onv=onv)
