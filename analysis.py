...
from models.ls import Ols
from models.misc import rmse, qlike
...
def run_analysis(rv):
    ...
    max_delta = 30
    hd = Hurst(np.sqrt(rv), q_list, max_delta)
    hd_est = hd.est_h()
    nud_est = hd.est_nu()
    ...
    print(f'Daily Series Hurst: {hw_est}, Nu: {nuw_est}')
    print(f'Daily Series Hurst: {hm_est}, Nu: {num_est}')
    ...
    roughvol_fc = roughvol.moving_window_lpred(train_size=train_size, bt=False)
    rhar_d = RoughVolatility(rv, hd_est, error_rhar, nud_est)
    rhar_x_d = rhar_d.moving_window_lpred(train_size=train_size, bt=False)
    rhar_w = RoughVolatility(prv_w, hw_est, error_rhar, nuw_est)
    rhar_x_w = rhar_w.moving_window_lpred(train_size=train_size, bt=False)
    rhar_m = RoughVolatility(prv_m, hm_est, error_rhar, num_est)
    rhar_x_m = rhar_m.moving_window_lpred(train_size=train_size, bt=False)

    # Align starting index and get variables
    # y = rv[522:]
    # har_x_d = rv[521: len(rv) - 1]
    # har_x_w = prv_w[517: len(prv_w) - 1]
    # har_x_m = prv_m[500: len(prv_m) - 1]
    # roughvol_fc = roughvol_fc[21: len(roughvol_fc) - 1]
    # rhar_x_d = rhar_x_d[21: len(rhar_x_d) - 1]
    # rhar_x_w = rhar_x_w[17: len(rhar_x_w) - 1]
    # rhar_x_m = rhar_x_m[: len(rhar_x_m) - 1]
    y = rv[521:]
    har_x_d = rv[520: len(rv) - 1]
    ...
    rhar_x_m = rhar_x_m[:]
    # har_x_d = np.log(har_x_d)
    # har_x_w = np.log(har_x_w)
    # har_x_m = np.log(har_x_m)
    ...
    roughvol_predicted = roughvol.back_transform(roughvol_predicted)
